...
def main():
    ...
    if st.button("결과 출력"):
        ...
        for group_name, q_ids in group_mapping.items():
            ...
            for qid in q_ids:
                ...
                if question:
                    ...
                    result_lines.append(f"{question['question_en']} – {question['description_en']}")
                    result_lines.append(f"({selected_score}) {question['choices'][selected_score]['en']} – {question['choices'][selected_score]['response_text_en']}")
        ...
        if impairment_question:
            ...
            result_lines.append(f"({selected_impairment}) {impairment_question['choices'][selected_impairment]['en']} – {impairment_question['choices'][selected_impairment]['response_text_en']}")
            result_lines.append("")
        ...
        # 클립보드 복사를 위한 버튼 (자바스크립트 사용)
        if st.button("클립보드 복사"):
            components.html(f"""
            <script>
            navigator.clipboard.writeText({result_text!r});
            </script>
            """, height=0)
